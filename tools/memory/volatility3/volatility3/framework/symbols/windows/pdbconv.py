...
if __name__ == "__main__":
    ...
    if args.output is None:
        ...
        # 获取数据库名称（PDB文件名）
        database_name = converted_json["metadata"]["windows"]["pdb"]["database"]
        if database_name == "unknown.pdb":
            # 如果无法从metadata获取，尝试从文件路径提取
            if args.file:
                database_name = os.path.basename(os.path.dirname(os.path.dirname(args.file)))

        # 创建输出目录结构：database_name/guid-age.json.xz
        output_dir = database_name  # 使用数据库名称作为目录名
        os.makedirs(output_dir, exist_ok=True)
        args.output = os.path.join(output_dir, f"{guid}-{age}.json.xz")
