...
@click.option(
    "--selection",
    type=click.Choice(["roulette", "tournament", "rank"]),
    default="tournament",
    show_default=True,
    help="Selection method for parent selection.",
)
@click.option(
    "-m",
    "--mutation",
...
def run(
    ...
    input_puzzle = cv.imdecode(np.fromfile(puzzle, dtype=np.uint8), cv.IMREAD_COLOR)
    ...
    click.echo(f"Mutation rate: {mutation}")
