...
class Crossover(object):
    ...
    def _get_best_match_piece(self, piece_id, orientation):
        candidates = ImageAnalysis.best_match_table[piece_id][orientation]
        # Stochastic element: 30% chance to pick 2nd-5th best match
        # to escape local optima more aggressively
        if len(candidates) > 1 and random.random() < 0.30:
            offset = random.randint(1, min(5, len(candidates) - 1))
            for piece, dissimilarity_measure in candidates[offset:]:
                if self._is_valid_piece(piece):
                    return piece, dissimilarity_measure

        for piece, dissimilarity_measure in candidates:
            if self._is_valid_piece(piece):
                return piece, dissimilarity_measure
