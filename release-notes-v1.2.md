...
- 已生成 Windows WebView EXE 发布包，双击 EXE 会启动本地 WebUI。
...
直接运行发布包中的 EXE 会打开 WebView 界面。
如需从源码运行，可使用 Python 启动方式：

```bat
start_webui.bat
```

源码运行前请确认 `venv` 已按 `requirements.txt` 安装依赖，前端资源已构建到 `frontend/dist`。
